...
public class Heap {
    ...
    private int leftChildIndex(int index) {
        if (valueList.size() > (2 * index) + 1) {   // TODO: bugfix #5/6 (it goes as third fixed bug) changed
            return (2 * (index) + 1);               //  "(2 * index) - 1)" to "(2 * index) + 1)"
        } else return -1;
    }
    ...
    private int elemParentIndex(int index) {
        if (index > 0) {
            return (index - 1) / 2;   //TODO: example bug fix (#1/6)
        } else return -1;
    }
        ...
        if (valueList.size() > index) {
            ...
            if (currentElem < leftChild) {
                if (leftChild > rightChild) {
                    valueList.set(index, leftChild);     // TODO: bugfix #2/6: valueList.set(index, rightChild); -> valueList.set(index, leftChild);
                    valueList.set(leftChildIndex, currentElem);
                    BubbleDown(leftChildIndex);
                } else {    // currentElem < leftChild < rightChild => the same as rightChild > currentElem => the logic must be the same
                    valueList.set(index, rightChild);
                    valueList.set(rightChildIndex, currentElem);
                    BubbleDown(rightChildIndex);    // TODO: bugfix #3/6 (added the entire line)
                }
            } else {
    ...
    private void BubbleUp(int index) {
        if (valueList.size() > index && elemParentIndex(index) > -1) {
            ...
            if (currentElem > parent) {
                valueList.set(index, parent);   // TODO: bugfix #4/6: valueList.set(parent, index); -> valueList.set(index, parent);
    ...
    public void removeElem(int elem) {
        ...
        if (indexOfRemoveElem == valueList.size() - 1) {
            ...
            valueList.remove(valueList.size() - 1);     // aTODO here will be the last bug
            valueList.set(indexOfRemoveElem, lastElem);
            BubbleDown(indexOfRemoveElem);      // TODO it is the last bugfix
